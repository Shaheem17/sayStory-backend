...
router.post("/insertStoryExist", function (request, response) {
  const authorDetails = {
...
router.get("/storyStatus", function (request, response) {
  const authorDetails = {
    authorId: request.body.authorID,
    secretPin: request.body.secretpin,
  };
...
router.get("/storyDetails", function (request, response) {
  try {
